...
class AIProcessor:
    ...
    def __init__(self, model: str = 'nous-hermes2:latest', base_url: str = 'http://localhost:11434', max_articles_per_batch=15):
        ...
        self.max_articles_per_batch = max_articles_per_batch  # NOUVEAU: Limite par batch
    ...
    def _query_ollama(self, prompt: str, max_tokens: int = 500) -> str:
        ...
        try:
            # NOUVEAU: Vérification de la taille du prompt
    ...
    def categorize_articles(self, articles: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        # NOUVEAU: Protection contre trop d'articles
        ...
        # NOUVEAU: Toujours utiliser le traitement par lots pour éviter les surcharges
        if len(articles) > 5:
...
Réponse (juste les catégories):"""
    ...
    def _batch_categorize_articles(self, articles: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        try:
            # NOUVEAU: Limite stricte du nombre d'articles
            if len(articles) > self.max_articles_per_batch:
                articles = articles[:self.max_articles_per_batch]
                logger.info(f"Limitation à {len(articles)} articles pour le batch")
            
            # NOUVEAU: Prompt plus court et efficace
            articles_list = []
...
Réponse:"""
            ...
            # NOUVEAU: Protection contre les prompts trop longs
            if len(prompt) > 6000:
    ...
    def synthesize_articles(self, articles: List[Dict[str, Any]]) -> str:
        ...
        try:
            ...
            # NOUVEAU: Résumés plus courts pour éviter les prompts longs
            articles_summary = []
            ...
            # NOUVEAU: Prompt simplifié et plus court
...
Max 200 mots total."""
            ...
            # NOUVEAU: Vérification taille prompt
            if len(prompt) > 4000:
